...
class LazyEmbeddingFunction:
    """懒加载的嵌入函数（GPU加速，首次使用时加载模型）"""

    def __init__(self, model_name: str = None, device: str = None):
        model_name = model_name or EMBEDDING_MODEL_NAME
        self.model_name = model_name
        self._model = None
        # 优先使用 GPU，兼容无 CUDA 环境
        if device:
            self._device = device
        else:
            import torch
            self._device = 'cuda' if torch.cuda.is_available() else 'cpu'
        logger.info(f"LazyEmbeddingFunction 初始化: {model_name} on {self._device} (模型未加载)")

    def __call__(self, input: List[str]) -> List[List[float]]:
        """首次调用时加载模型到 GPU"""
        if self._model is None:
            logger.info(f"首次使用，正在加载嵌入模型: {self.model_name} → {self._device}")
            from sentence_transformers import SentenceTransformer
            self._model = SentenceTransformer(self.model_name, device=self._device)
            logger.info(f"嵌入模型加载完成: {self.model_name} @ {self._device}")
    ...
    def name(self) -> str:
        """ChromaDB 兼容：返回嵌入函数名称（与 import 脚本的 SentenceTransformerEmbeddingFunction 一致）"""
        return "sentence_transformer"
    
    def embed_query(self, input=None, **kwargs):
        """ChromaDB 兼容：查询嵌入（chromadb 调用 embed_query(input=["text"])）"""
        if input is None:
    ...
    def unload(self):
        ...
        if self._model is not None:
            logger.info(f"释放嵌入模型: {self.model_name}")
            del self._model
            self._model = None
            gc.collect()
            logger.info("嵌入模型已释放")
    ...
    def _init_embedding_function(self):
        ...
        try:
            ...
            if EMBEDDING_MODEL['provider'] == 'openai':
                ...
                # 使用懒加载的 sentence-transformers (从 config 读取模型名和设备)
                self.embedding_function = LazyEmbeddingFunction(device=EMBEDDING_DEVICE)
                logger.info(f"使用 {EMBEDDING_MODEL_NAME} Embedding 模型 (懒加载, device={EMBEDDING_DEVICE})")
