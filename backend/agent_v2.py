    ...
    if _llm_service is None:
        ...
        try:
            if LLM_PROVIDER == 'ollama':
                from llm_service import ollama_llm_service
                _llm_service = ollama_llm_service
            else:
...
class AISpecialtyAgentV2:
    ...
    async def process_query(
        ...
        if llm and llm.is_available():
            try:
                ...
                raw_response = await llm.chat(query, context, session_history, system_prompt=cot_prompt)
                if raw_response:
        ...
        if enable_thinking and used_llm and sources and llm and llm.is_available():
            try:
                reflection_result, revised_response = await self._self_rag_reflect(
                    query, response, sources, llm,
                )
        ...
        return {
            ...
            'metadata': {
                ...
                'crag_evaluation': {
                    'quality_score': crag_info.get('quality_score'),
                    'action': crag_info.get('action'),
                } if crag_info else None,
    ...
    def _build_thinking_process(
        ...
        rerank_step = ThinkingStep(
            step_id=3,
            step_name='上下文整理',
            description='去重、排序、CRAG 质量检验并裁剪上下文',
        ...
        if reflection_result and reflection_result.status != 'skipped':
            ...
            reflection_step = ThinkingStep(
                step_id=5,
                step_name='Self-RAG 反思',
                description='验证回答与来源证据的一致性',
